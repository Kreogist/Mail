...
QColor KMUtil::parseColor(const QString &data)
{
    //Check data length.
    switch(data.length())
    {
...
QString KMUtil::ensurePathValid(const QString &path)
{
    ...
    if(detectInfo.isFile())
    {
        ...
        {
            //If we cannot remove the file which has the same name, then failed.
            return QString();
        }
    }
    //Generate the folder.
...
QString KMUtil::simplifiedPath(const QString &path)
{
